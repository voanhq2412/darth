...
# The performance of Machine Learning models is highly dependent on their hyperparameters. Neural Network’s flexibility also means that there are many hyperparameters to tune. These may include the number of layers, the type of layers and the ordering of layers; tweaking these hyperparameters can also be refered to as tweaking the architecture of the network. 
...
# 
# However, most of these methods incoporate additional rule-based or randomized architecture regularization technqiues. Searching for the best set of rules or regularization techniques is akin to manually searching for the optimal architecture for a neural network, which defeats the purpose. 
...
# | Beta-DARTS | 1) Explicit regularization of softmax-activated architecture parameter $\beta$ (rather than  the non-activated $\alpha$, since the searching and discretization process of DARTS rely on Beta to represent the importance of each operator. This improves the robustness of DARTS. Don't need to rely on arbitary dropping rules| An Improvement over other regularization method. Still regularization should only be an after thought, more attention should be given to problem formulation, gradient approximation and search space analysis.|
...
# 
# The bilevel optimization is formulated as:<br>
# $min_{\alpha} \ \ L_{val} (w^*(\alpha),\alpha) + \dfrac{r}{2}. \alpha^T.\alpha$<br> 
